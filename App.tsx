      ...
      switch (type) {
        case 'load':
          ...
          if (append) {
             ...
             allTranslations.forEach(item => {
                 ...
                 if (k !== undefined && k !== null && k !== '') {
                     ...
                     existingMap.set('__nokey_existing_' + (noKeyCounter++), item);
                 }
             });
             data.forEach(item => {
                 const k = item.key;
                 if (k !== undefined && k !== null && k !== '') {
                     ...
                     existingMap.set('__nokey_new_' + (noKeyCounter++), item);
                 }
             });
             allTranslations = Array.from(existingMap.values());
          } else {
          ...
          for (let i = 0; i < allTranslations.length; i++) {
            const keys = Object.keys(allTranslations[i]);
          ...
          <div className="space-y-6">
             <div className="bg-gray-800 p-6 rounded-lg shadow-xl space-y-6 border border-gray-700">
                <div>
                    ...
                    <div onDragOver={(e)=>{e.preventDefault();setIsDragging(true)}} onDragLeave={()=>setIsDragging(false)} onDrop={(e)=>{e.preventDefault();setIsDragging(false);const f=e.dataTransfer.files;if(f.length)processFiles(Array.from(f), false)}}
                        ...
                        <div className="space-y-1 text-center">
                            ...
                            <div className="flex text-sm text-gray-400 justify-center">
                                <label className="relative cursor-pointer text-cyan-400 hover:text-cyan-300 font-medium">
                                    ...
                                    <input type="file" className="sr-only" accept=".json" multiple onChange={(e)=>e.target.files?.length && processFiles(Array.from(e.target.files), false)}/>
                                </label>
                      ...
                      <div className="mt-3 space-y-2">
                        ...
                        <div className="flex gap-2">
                            <label className="cursor-pointer text-xs bg-gray-700 hover:bg-gray-600 px-3 py-1.5 rounded text-white font-medium transition-colors">
                                Append File(s)
                                <input type="file" className="sr-only" accept=".json" multiple onChange={(e)=>e.target.files?.length && processFiles(Array.from(e.target.files), true)}/>
                            </label>
                        </div>
                      </div>
                    )}
